...
### ℹ️ How URLs are Constructed
In FastAPI, we define "Routers" in separate files (like `front_auth.py`), and then "mount" them in `main.py` with a **Prefix**.
- `front_auth.py` has `@router.post("/register")`.
- `main.py` mounts it with `app.include_router(..., prefix="/auth")`.
- **Final URL**: `/auth` + `/register` = `/auth/register`.
...
## 3. Sellers / Agents (`/stats`)
Defined in `app/routers/sellers.py`, but mounted with prefix `/stats` in `main.py`.

### List Online Agents
Get a list of currently active agents (sellers) available for work.

- **Endpoint**: `GET /stats/agents/online`
- **Response**: `200 OK`
  ```json
  [
    {
...
### D. Finding Sellers
1. **Show Network Status**: `GET /stats/agents/online`.
